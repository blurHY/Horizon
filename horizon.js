...
class Page extends ZeroFrame {
	...
	onOpenWebsocket() {
		...
		$(".help").html(marked(`
### Examples
"search engine" filter:uniquetitle
talk -FAQ -nettalk -test
### About
...
`))
	...
	generateWheres(fields) { // include > exclude > normal (priority)
		...
		for (const a of this.searchq) {
			...
			let likes = []
			for (let f of fields)
				likes.push(`lower(${f}) like "%${es}%" escape "\\"`)
			if (likes.length > 1)
		...
		for (const a of this.excludekws) {
			...
			let likes = []
			for (let f of fields)
				likes.push(`lower(${f}) not like "%${es}%" escape "\\"`)
			if (likes.length > 1)
		...
		if (whereand.length > 1)
			final = whereand.join(") and (")
		else
			final = whereand

		if (whereor)
			final = `(${final}) or (${whereor})`
		else if (whereand.length > 1)
			final = "(" + final + ")"
	...
	genWikiQuery() {
		...
		for (let x of this.searchq) {
			conds.push(`lower(slug) like "%${escapeSql(x.toLowerCase())}%" escape "\\"`)
			conds.push(`lower(slug) like "%${escapeSql(encodeURIComponent(x.toLowerCase()).toLowerCase())}%" escape "\\"`) // Some Chinese wiki encoded slug
			conds.push(`lower(slug) like "%${escapeSql(encodeURIComponent(x.toUpperCase()).toLowerCase())}%" escape "\\"`)
		}
	...
	generateItem(title, imgc, id, link, keywords, phrases, item) {
		...
		// 	ele.find(".type").text("ZeroTalk").addClass("orange")
		// else if (typeof id !== "undefined" && this.searchres[id].originalZblogObj)
		// 	ele.find(".type").text("ZeroBlog").addClass("purple")
		// else if (typeof id !== "undefined" && this.searchres[id].originalZeroupObj)
		// 	ele.find(".type").text("ZeroUp").addClass("blue")
		...
		if (item) {
			...
			if (item.originalZeroupObj) {
				...
				type.text("ZeroUp").addClass("blue")
			} else ele.find(".zuinfo").remove()

			if (item.originalZblogObj) {
				ele.find(".zuinfo").show()
				ele.find(".zusize").text(humanFileSize(item.originalZeroupObj.size))
				ele.find(".zudate").text(timeagoinstance.format(item.originalZeroupObj.date_added * 1000, "en-US"))
				ele.find(".zuuser").text(item.originalZeroupObj.cert_user_id)
				type.text("ZeroBlog").addClass("purple")
			} else ele.find(".zuinfo").remove()
		}
		if (type.text() === "")
			type.remove()
	...
	showMoreInfo(ev) {
		...
		if (typeof id !== "undefined") {
			...
			if (page.searchres[id].originalGroupObj) {
				...
				$(".kws").empty()
				let fullkw = page.getFullKeywords(id)
				if (fullkw.length > 0) {
					for (let r of fullkw)
				...
				$(".phs").empty()
				let fullph = page.getFullPhrases(id)
				if (fullph.length > 0) {
					for (let r of fullph)
