...
### About
I'm planing to write a dectranlized crawler for zeronet.
But it needs to optimize zeronet (crawled data is big)
So i'm going to learn more about python.
...
`))
	...
	genQuery(type, siteaddr) {
		switch (type) {
			...
			case "ZeroBlog":
				return this.genZeroBlogQuery("and", siteaddr)
			case "ZeroUp":
	...
	genWikiQuery(siteaddr) {
		...
		return `select body,date_added,slug,"${escapeSql(siteaddr)}" as siteaddr from pages where date_added in (select max(date_added) from pages where ${conds.join(" or ")} group by slug limit 2) order by length(body) desc,date_added desc limit 2`
	}

	genZeroBlogQuery(mode = "and", siteaddr) {
		return `select *,"${escapeSql(siteaddr)}" as siteaddr from post where ${this.generateLikes("body", "title", mode)} order by ${this.generateOrderBys(["title", "body"])}`
	}

	genZeroUpQuery(siteaddr) {
		return `select *,"${escapeSql(siteaddr)}" as siteaddr from file left join json using(json_id) where ${this.generateLikes("file.title", "file.file_name", "or")} order by ${this.generateOrderBys(["file.title", "file.file_name"])} Limit 1000`
	}
	...
	parseQuery() {
		this.querySplited = this.searchQuery.split(/\s+/g)
	...
	generateLikes() {
		let qs = []
		let mode = "and"
		if (arguments[arguments.length - 1] === "or" || arguments[arguments.length - 1] === "and") {
		...
		switch (type) {
			case "normal":
				query = `select 
						...
						,main
						where firstres.id=main.id and ${this.generateLikes("title", "keyw", "phrs")} 
	...
	showOnePage() {
		...
		if (this.searchResult.length < 1) {
			cot.append(this.generateItem("Sorry,noting found", 0, null, "/", "Please type fewer keywords.Use white space to separate them"))
		}
		if (this.pagenum > 0) {
			$("div.stat").hide()
			page.clearCards()
