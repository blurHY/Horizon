...
`))
		...
		$(document.body).click(function (ev) {
			...
			if (ev.target.tagName === "A")
				page.cmd("wrapperOpenWindow", [$(ev.target).data("href"), "_blank"])
		})
		...
		$(".q").keydown(function (ev) {
			if (ev.key === "Enter" && $(this).val().trim() !== "") {
				try {
					...
					showError("Please report : " + JSON.stringify(e))
				}
			}
		})
	}
			...
			case "normal":
				query = `select 
						...
						(
							...
							from 
								(
									select * from (select id,NULL as keyw,NULL as phrs from main where ${this.generateLikes("url", "title", this.fuzzysearch ? "or" : "and")} limit 3000)
									union all
									select * from (select pageid as id,keyword as keyw,NULL as phrs from keywords where ${this.generateLikes("keywords.keyword", "or")} limit 3000)
									union all
									select * from (select pageid as id,NULL as keyw,phrase as phrs from phrases where ${this.generateLikes("phrases.phrase", this.fuzzysearch ? "or" : "and")} limit 3000)
								)
	...
	generateCard(item, markdown = false) {
		...
		card.find(".cardtitle").text(item.title).data("href", item.link)
		if (!markdown)
	...
	generateItem(title, imgc, id, link, description) {
		...
		ele.find("a.ititle").html(this.highlightedText(title)).data("href", link)
	...
	mergeItems_addData(items) {
		for (let i = 0; i < items.length; i++) {
			if (!items[i])
				continue
			switch (this.getItemType(items[i])) {
				case "normal":
					let siteroot = getSiteRootUrl(items[i].url)
					items[i].siteroot = siteroot
					let del = false
					if (this.needToQuery_zites.has(siteroot)) // Reatin the first one
						del = true
	...
	showOnePage() {
		...
		this.emptyItem()
		this.needToQuery_zites_obj = {}
		this.needToQuery_zites = new Set()
		scrollTo(0, 0)
		...
		for (let i of items) {
			...
			switch (this.getItemType(i)) {
				case "normal":
					...
					item.data("siteaddr", getSiteRootUrl(i.url))
	...
	showMoreInfo(ev) {
		...
		if (typeof id !== "undefined") {
			...
			if (siteaddr) {
				let merged = mitem.find(".merged").show()
				...
				for (let i = 0; i < page.needToQuery_zites_obj[siteaddr].length; i++) {
					...
					ele.find(".link").text(href).data("href", href)
					mergedurls.append(ele)
				}
				mitem.find(".column").show()
				page.getWordCloudDataZite(siteaddr, wordlist => {
					let wordc = mitem.find(".wordcloud")
					console.log(wordlist)
					if (wordlist.length > 0) {
						notempty = true
						WordCloud(wordc.find("canvas")[0], {
							list: wordlist,
							weightFactor: pt => (pt > 1) ? (pt * 1.5) : (pt * 0.7)
						})
						wordc.show()
					}
					else
			...
			function checkEmpty() {
				if (!notempty) {
					mitem.find(".coloum").hide()
					mitem.find(".nomoredata").show()
				} else {
					mitem.find(".column").show()
					mitem.find(".nomoredata").hide()
				}
			}
		}
	}
